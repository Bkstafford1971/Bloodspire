...
ipcMain.handle('data:getManagerRecord', async (event, managerName) => {
  ...
  try {
    ...
    const raceRecords = {};
    const raceVsRace = {};
    const weaponRecords = {}; // Track weapon performance
    ...
    if (fs.existsSync(leagueResultsDir)) {
      const turnDirs = fs.readdirSync(leagueResultsDir).filter(f => f.startsWith('turn_'));
      for (const turnDir of turnDirs) {
        const turnPath = pathLib.join(leagueResultsDir, turnDir);
        if (!fs.statSync(turnPath).isDirectory()) continue;
        const resultFiles = fs.readdirSync(turnPath).filter(f => f.startsWith('result_') && f.endsWith('.json'));
        ...
        if (teamData.warriors) {
          for (const warrior of teamData.warriors) {
            ...
            for (const fight of warrior.fight_history) {
              ...
              const weaponUsed = fight.weapon_name || 'Unknown';
              if (fight.weapon_name && fight.weapon_name !== 'Unknown') {
                console.log(`[DEBUG] Fight: ${warrior.name} vs ${fight.opponent_name}, weapon: ${fight.weapon_name}`);
              }
              ...
              if (!weaponRecords[weaponUsed]) {
                weaponRecords[weaponUsed] = { wins: 0, losses: 0, kills: 0, race_breakdown: {} };
              }
              ...
              if (fight.result === 'win') {
                ...
                weaponRecords[weaponUsed].losses++;
                totalLosses++;
              }
              ...
              if (fight.opponent_slain) {
                ...
                weaponRecords[weaponUsed].kills++;
                totalKills++;
              }
              ...
              // Track race breakdown for this weapon
              if (!weaponRecords[weaponUsed].race_breakdown[opponentRace]) {
                weaponRecords[weaponUsed].race_breakdown[opponentRace] = {
                  wins: 0, losses: 0, kills: 0, warrior_breakdown: {}
                };
              }
              if (fight.result === 'win') {
                weaponRecords[weaponUsed].race_breakdown[opponentRace].wins++;
              } else if (fight.result === 'loss') {
                weaponRecords[weaponUsed].race_breakdown[opponentRace].losses++;
              }
              if (fight.opponent_slain) {
                weaponRecords[weaponUsed].race_breakdown[opponentRace].kills++;
              }

              // Track warrior breakdown for this weapon + race
              const opponentWarrior = fight.opponent_name || 'Unknown';
              if (!weaponRecords[weaponUsed].race_breakdown[opponentRace].warrior_breakdown[opponentWarrior]) {
                weaponRecords[weaponUsed].race_breakdown[opponentRace].warrior_breakdown[opponentWarrior] = {
                  wins: 0, losses: 0, kills: 0
                };
              }
              if (fight.result === 'win') {
                weaponRecords[weaponUsed].race_breakdown[opponentRace].warrior_breakdown[opponentWarrior].wins++;
              } else if (fight.result === 'loss') {
                weaponRecords[weaponUsed].race_breakdown[opponentRace].warrior_breakdown[opponentWarrior].losses++;
              }
              if (fight.opponent_slain) {
                weaponRecords[weaponUsed].race_breakdown[opponentRace].warrior_breakdown[opponentWarrior].kills++;
              }
            }
          }
        }
    ...
    // Convert weapon records to sorted array format with nested race and warrior breakdowns
    const weaponRecordsArray = Object.entries(weaponRecords)
      .map(([weapon, stats]) => ({
        ...
        losses: stats.losses,
        kills: stats.kills,
        race_breakdown: Object.entries(stats.race_breakdown)
          .map(([race, raceStats]) => ({
            race,
            wins: raceStats.wins,
            losses: raceStats.losses,
            kills: raceStats.kills,
            warrior_breakdown: Object.entries(raceStats.warrior_breakdown)
              .map(([warrior, warriorStats]) => ({
                warrior,
                wins: warriorStats.wins,
                losses: warriorStats.losses,
                kills: warriorStats.kills
              }))
              .sort((a, b) => (b.wins + b.losses) - (a.wins + a.losses))
          }))
          .sort((a, b) => (b.wins + b.losses) - (a.wins + a.losses))
    ...
    // Add "Unknown" total record if not present
    if (!weaponRecordsArray.find(w => w.weapon === 'Unknown')) {
      weaponRecordsArray.push({
        weapon: 'Unknown',
        wins: totalWins - weaponRecordsArray.reduce((sum, w) => sum + w.wins, 0),
        losses: totalLosses - weaponRecordsArray.reduce((sum, w) => sum + w.losses, 0),
        kills: totalKills - weaponRecordsArray.reduce((sum, w) => sum + w.kills, 0),
        race_breakdown: []
      });
    }
