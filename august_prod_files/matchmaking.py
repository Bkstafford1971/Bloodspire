...
def _recent_opponent_names(warrior: Warrior, lookback: int = 3) -> set:
    """Names of opponents this warrior fought in the last `lookback` completed turns."""
    return {
        entry.get("opponent_name")
        for entry in warrior.fight_history[-lookback:]
...
def build_global_fight_card(
    ...
    for t in unique_teams:
        for w in t.active_warriors:
            if w and getattr(w, 'current_arena', 'normal') == arena:
    ...
    for idx, entry in enumerate(list(unmatched)):
        ...
        if eligible:
            ...
            # Prefer opponents NOT fought in the last 3 turns to avoid immediate rematches
            recent = _recent_opponent_names(entry['warrior'], lookback=3)
            non_repeat = [e for e in eligible if e['warrior'].name not in recent]
            pool = non_repeat if non_repeat else eligible  # Fall back to eligible if no fresh opponents
            # Prefer the closest fight-count match first; break ties on overall
            # rating closeness (stats/skills), same as before.
            pool.sort(key=lambda e: (abs(e['warrior'].total_fights - self_fights),
                                      abs(_warrior_rating(e['warrior']) - self_rating)))
            _add_fight(entry, pool[0], "standard")
...
def _find_opponent(
    ...
    for best_team in candidates:
        ...
        # Prefer opponents NOT fought in the last 3 turns to avoid immediate rematches
        recent = _recent_opponent_names(player_warrior, lookback=3)
        non_repeat = [w for w in pool if w.name not in recent]
        final_pool = non_repeat if non_repeat else pool  # Fall back to pool if no fresh opponents
        # Prefer the closest fight-count match first; break ties on overall
        # rating closeness (stats/skills), same as build_global_fight_card's
        # random P-vs-P step.
        final_pool.sort(key=lambda w: (abs(w.total_fights - player_warrior.total_fights),
