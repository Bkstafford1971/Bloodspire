...
class CombatDebugLogger:
    ...
    def log_action_start(self,
        ...
        is_counterstrike = "Counterstrike" in style
        ...
        if ia is not None and ia_comps is not None:
            ...
            if is_counterstrike:
                self._emit(
                    f"  Initiative: {attacker_name.upper()} gets COUNTERSTRIKE"
                    f" (automatic riposte, no roll needed)"
                )
            else:
                self._emit(
    ...
    def log_counterstrike_attack_rolls(self, attacker_name: str,
                                       roll_1: int, roll_2: int,
                                       selected_roll: int, final_result: int):
        self._emit(f"  Counterstrike Attack Roll ({attacker_name.upper()}) - WITH ADVANTAGE:")
        self._emit(f"    Roll 1: {roll_1}")
        self._emit(f"    Roll 2: {roll_2}")
        self._emit(f"    ▶ Selected (higher): Roll {selected_roll} = {final_result}")
