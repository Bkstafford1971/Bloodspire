...
class Team:
    ...
    def total_wins(self) -> int:
        active = sum(w.wins for w in self.warriors if w)
        archived = sum(w.get("wins", 0) for w in self.archived_warriors)
        return active + archived

    @property
    def total_losses(self) -> int:
        active = sum(w.losses for w in self.warriors if w)
        archived = sum(w.get("losses", 0) for w in self.archived_warriors)
        return active + archived

    @property
    def total_kills(self) -> int:
        active = sum(w.kills for w in self.warriors if w)
        archived = sum(w.get("kills", 0) for w in self.archived_warriors)
        return active + archived

    @property
    def record_str(self) -> str:
        """Returns the aggregate team record: Wins-Losses-Kills."""
        return f"{self.total_wins}-{self.total_losses}-{self.total_kills}"
...
#    battle_cry)                                     # shouted at fight start
#
...
PEASANT_ROSTER = [
    # Biff Watson: knuckle fighter, raw power with hand combat
    ("Biff Watson",  "Male",   "Brigandine",  "Cestus",
     (14, 11, 13,  9,  8, 16),
     (22, 17, 20, 12, 10, 16),
     5, 3, "parry",
    ...
    ("The Skunk Lady",            "Female", "Cuir Boulli", "Short Sword",
     ...
     (15, 21, 15, 17, 13, 12),
     5, 4, "dodge",
     "DO YOU SMELL THAT?  IS THAT ME?"),
    # Sheriff Craig: balanced DEX/INT, surprisingly capable with a Club
    ("Sheriff Craig",  "Male",   "Leather",     "Club",
     (12, 13, 12, 11,  9, 15),
     (17, 19, 17, 15, 12, 15),
     5, 3, "parry",
    ...
    ("Matt the Basement Dweller",  "Male", "Brigandine",  "Flail",
     ...
     (19, 15, 21, 12, 11, 16),
     4, 3, "parry",
     "THE MAGIC FOX TELLS ME EVERYTHING I NEED TO KNOW!"),
    ...
    ("Spooky Wendell", "Male",   "Brigandine",       "Scythe",
     (13,  9, 12,  8,  7, 16),
     (18, 14, 18, 11,  9, 16),
     4, 2, "parry",
    ...
    ("The Redmasked Madman", "Male",   "Leather",       "Short Sword",
     (10, 11, 11, 11, 14, 14),
     (14, 15, 16, 17, 14, 14),
     4, 3, "parry",
     "AFTER THIS FIGHT, I'LL BE MAKING ROUGH LOVE TO A TREE!  WAAAAHAHAHAHAOOOOOO!"),
    # Payne the Punisher: DEX heavy but fragile and unpredictable
    ("Payne the Punisher",        "Male",   "Scale",       "Pole Axe",
     ( 11, 12, 10,  9, 10, 12),
     (14, 18, 14, 13, 10, 12),
     4, 2, "dodge",
     "THE CROWD HAS DECIDED IT IS TIME FOR A HEAD TO ROLL.....YOURS!!"),
    # Wilbur the Weed-Puller: weak all-around but not totally hopeless
    ("Wilbur the Weed-Puller","Male",   "Cloth",       "Short Sword",
     ( 8,  9,  9,  8,  6, 12),
     (12, 13, 13, 12, 10, 12),
     2, 2, "parry",
     "I'm... probably better than I look. Probably."),
]
...
def _make_peasant(roster_index: int, experience_level: int = 0) -> Warrior:
    ...
    name, gender, armor, weapon, base_stats, max_stats, wpn_sk_max, def_sk_max, def_sk_key, battle_cry = \
        PEASANT_ROSTER[roster_index]
    if isinstance(battle_cry, (list, tuple)):
        battle_cry = random.choice(battle_cry)
    ...
    w.battle_cry = battle_cry

    from warrior import Strategy
    w.strategies = [
        Strategy(trigger="Always (Default Loop)", style="Strike", activity=5, aim_point="Chest", defense_point="Chest"),
        Strategy(trigger="You are on the ground", style="Wall of Steel", activity=5, aim_point="Chest", defense_point="Chest"),
        Strategy(trigger="You have taken heavy damage", style="Engage & Withdraw", activity=6, aim_point="Chest", defense_point="Chest"),
    ]
    return w
