...
def _generate(uploads: dict, team_map: dict, turn_num: int, output_dir: str) -> None:
    ...
    # Career totals are sourced from standings.json rather than the current
    # team uploads. Unlike the uploads (which only cover live rosters plus
    # each team's own fallen_warriors log), standings.json is a persistent,
    # cross-turn ledger keyed by warrior_id that already folds in archived
    # (dead-and-replaced) warriors, so it survives roster resets and gives
    # the true career picture instead of just what's on the roster right now.
    standings = _load_standings_data()
    for team_data in standings.values():
        for ws in (team_data.get("warriors") or {}).values():
            race   = ws.get("race",   "")
            gender = ws.get("gender", "")
            w = ws.get("wins",   0)
            l = ws.get("losses", 0)
            k = ws.get("kills",  0)
            if race in career_race:
                career_race[race]["w"] += w
                career_race[race]["l"] += l
                career_race[race]["k"] += k
            if gender in career_gender:
                career_gender[gender]["w"] += w
                career_gender[gender]["l"] += l
                career_gender[gender]["k"] += k
    ...
    for team_key, team_data in standings.items():
        ...
        for upload_key, (result_data, result_turn) in results.items():
            if str(team_id) in upload_key:
                # Sum directly from each warrior's own cumulative wins/losses/kills
                # (active + archived) - NOT from turn_history, which team.to_dict()
                # caps to the last 20 turns and so silently drops earlier turns once
                # the season runs longer than that.
                result_team = result_data.get("team", {})
                for warrior in (result_team.get("warriors") or []):
                    if warrior:
                        w += warrior.get("wins", 0)
                        l += warrior.get("losses", 0)
                        k += warrior.get("kills", 0)
                for warrior in (result_team.get("archived_warriors") or []):
                    if warrior:
                        w += warrior.get("wins", 0)
                        l += warrior.get("losses", 0)
                        k += warrior.get("kills", 0)
...
def _get_top_warriors_by_popularity(standings, limit=10):
    """Get top warriors by popularity. Loads full warrior data from team files."""
    import json
    ...
    # Load all team files to get complete warrior data including popularity and is_dead
    try:
        for fname in os.listdir(teams_dir):
            ...
            try:
                with open(os.path.join(teams_dir, fname), "r", encoding="utf-8") as f:
                    team_data = json.load(f)
                    team_name = team_data.get("team_name", "?")

                    for warrior_data in team_data.get("warriors", []):
                        # Skip dead warriors
                        if warrior_data.get("is_dead"):
                            continue
