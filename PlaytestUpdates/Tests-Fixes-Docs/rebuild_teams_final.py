#!/usr/bin/env python3
"""
Rebuild with MERGED fight history AND recalculated total_fights.
"""
...
def rebuild_team_file(team_id, team_filename):
    """Rebuild team file with merged fight history and corrected stats."""
    print(f"\n{'='*60}")
    ...
    print(f"  Merging fight history from turns 1-3 and recalculating stats...")
    
    merged_fights = merge_fight_histories(team_id)
    
    # Update active warriors
    for warrior in team_data.get("warriors", []):
        if not warrior:
            continue
        ...
        if name in merged_fights:
            ...
            # Recalculate total_fights based on merged history
            warrior["total_fights"] = len(warrior["fight_history"])
    
    # Update archived warriors too
    ...
    for warrior in team_data.get("archived_warriors", []):
        ...
        fights = len(warrior.get("fight_history", []))
        total_fights += fights
        print(f"    {name} (archived): {fights} fights")
...
def main():
    print(f"\nTEAM REBUILD WITH CORRECTED STATS")
