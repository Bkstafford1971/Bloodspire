    ...
    try:
        ...
        for line in lines:
            ...
            if "NAME" in line and "W" in line and "L" in line and "K" in line and "REC" in line:
                ...
                continue
            elif in_warrior_section and (line.startswith("-") or not line.strip()):
                in_warrior_section = False
                continue

            if in_warrior_section and line.strip():
                # Parse warrior line: NAME W L K REC TEAM(ID)
                # Find the 4 consecutive numbers (W L K REC), everything before is name,
                # everything after stats before (ID) is team
                parts = line.split()

                if len(parts) >= 6:  # At least: NAME, W, L, K, REC, TEAM_WORD, (ID)
                    try:
                        # Find 4 consecutive numbers
                        stats_start_idx = -1
                        for i in range(len(parts) - 1):
                            # Check if this and next 3 are all numbers
                            if i + 3 < len(parts):
                                try:
                                    int(parts[i])
                                    int(parts[i + 1])
                                    int(parts[i + 2])
                                    int(parts[i + 3])
                                    # Check that the next part is not a number (should be team name)
                                    try:
                                        int(parts[i + 4])
                        ...
                        if stats_start_idx > 0:
                            # Extract stats
                            w_val = int(parts[stats_start_idx])
                            l_val = int(parts[stats_start_idx + 1])
                            k_val = int(parts[stats_start_idx + 2])
                            rec_val = int(parts[stats_start_idx + 3])

                            # Warrior name is everything before stats
                            ...
                            # Team name is between stats and team ID
...
def _get_top_warriors(standings, stat_field, limit=10):
    ...
    warriors = list(warriors_dict.values())

    # Sort by stat_field (descending), then by wins (descending), then by recognition (descending)
    # This creates a multi-level sort: primary by stat_field, secondary by wins, tertiary by recognition
    warriors.sort(key=lambda x: (-x[stat_field], -x["wins"], -x["recognition"]))

    # Take top limit warriors
...
def _get_top_warriors_by_kills(standings, limit=10):
    ...
    # Sort by kills (descending), then by wins (descending), then by recognition (descending)
...
def _generate_top_warriors_page(standings: dict, stat_field: str, title: str, turn_num: int) -> str:
    ...
    dead_rows = []
    for w in warriors:
        is_dead = w.get('recognition', 0) == 0
        manager_cell = w.get('manager', '?')
        if is_dead:
            manager_cell += ' <span style="color:#cc0000;font-weight:bold;font-size:0.85em">Dead</span>'
        rows.append([w['name'], w['team'], f"{w['wins']}-{w['losses']}-{w['kills']}", str(w.get('recognition', 0)), manager_cell])
        dead_rows.append(is_dead)
    return _render_stat_page(title, headers, rows, turn_num, dead_rows)
...
def _generate_top_warriors_by_kills_page(standings: dict, turn_num: int) -> str:
    ...
    for w in warriors:
        ...
        rows.append([w['name'], w['team'], f"{w['wins']}-{w['losses']}-{w['kills']}", str(w.get('recognition', 0)), manager_cell])
        dead_rows.append(is_dead)
    return _render_stat_page("Top 10 Warriors by Kills", headers, rows, turn_num, dead_rows)
...
def _get_top_warriors_by_losses(standings, limit=10):
    ...
    # Sort by losses (descending), then by wins (descending), then by recognition (descending)
...
def _generate_top_warriors_by_losses_page(standings: dict, turn_num: int) -> str:
    ...
    for w in warriors:
        ...
        rows.append([w['name'], w['team'], f"{w['wins']}-{w['losses']}-{w['kills']}", str(w.get('recognition', 0)), manager_cell])
        dead_rows.append(is_dead)
    return _render_stat_page("Top 10 Warriors by Losses", headers, rows, turn_num, dead_rows)
