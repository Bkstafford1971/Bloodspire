...
class StrategyOptimizer:
    ...
    def recommend_default_style(self) -> tuple:
        ...
        w = self.warrior
        mods = w.race.modifiers

        # Racial specials override attribute logic when the race has a defining ability
        if mods.counterstrike_mastery and w.dexterity >= 12:
            return ("Counterstrike", f"{w.race.name} counterstrike mastery — punish enemy aggression")

        if mods.thrown_mastery:
            throw_skill = w.skills.get("opportunity_throw", 0)
            if throw_skill >= 3:
                return ("Opportunity Throw", f"{w.race.name} thrown mastery, throw skill {throw_skill}/9")

        if mods.natural_weapon_bonus:
            mc_skill = w.skills.get("martial_combat", 0)
            if mc_skill >= 3:
                return ("Martial Combat", f"{w.race.name} natural weapon bonus, MC skill {mc_skill}/9")

        if mods.frenzy_ability and w.dexterity >= 13:
            return ("Lunge", f"{w.race.name} frenzy ability + DEX {w.dexterity} → explosive burst attacks")

        # Tabaxi/Goblin: avoid styles that imply heavy weapons
        if mods.heavy_weapon_penalty:
            if w.dexterity >= 14:
                return ("Calculated Attack", f"High DEX + {w.race.name} speed — precision over power")
            elif w.dexterity >= 12:
                return ("Strike", f"{w.race.name} — light weapon speed, no heavy-weapon styles")
    ...
    def recommend_activity(self) -> tuple:
        ...
        # Apply racial attack rate modifiers
        net_rate = mods.attack_rate_bonus - mods.attack_rate_penalty
        if net_rate > 0:
            adj = min(2, (net_rate + 1) // 2)
            activity = min(9, activity + adj)
            reason += f" + {w.race.name} speed (+{net_rate} rate)"
        elif net_rate < 0:
            # Half-Orc and similar: DEX tiers can offset the penalty
            dex_offset = sum(
                reduction for threshold, reduction in mods.dex_attack_rate_tiers.items()
                if w.dexterity >= threshold
            )
            remaining = abs(net_rate) - dex_offset
            if remaining > 0:
                adj = min(2, (remaining + 1) // 2)
                activity = max(0, activity - adj)
                reason += f" - {w.race.name} speed penalty (net -{remaining} after DEX)"
            else:
                reason += f" ({w.race.name} speed penalty offset by DEX {w.dexterity})"
    ...
    def _eligible_weapons(self) -> List[Tuple]:
        """Return (key, wpn, two_h_cap) for every weapon this warrior can legally use."""
        from weapons import WEAPONS, max_weapon_weight, get_effective_strength_for_weapons
        w = self.warrior
        mods = w.race.modifiers
        weak_set = set(mods.weak_weapons)
        eff_str = get_effective_strength_for_weapons(w)
        str_cap = max_weapon_weight(eff_str)
        HEAVY_THRESHOLD = 4.0

        result = []
        for key, wpn in WEAPONS.items():
            if wpn.display in weak_set:
                continue
            is_heavy = (wpn.weight >= HEAVY_THRESHOLD) or wpn.two_hand
            if mods.heavy_weapon_penalty and is_heavy:
                if not (mods.spear_exception and "Spear" in wpn.display):
                    continue
            two_h_cap = str_cap + (1.0 if wpn.two_hand else 0.0)
            if wpn.weight > two_h_cap + 1.5:
                continue
            result.append((key, wpn, two_h_cap))
        return result

    def _best_weapon_for_situation(self, situation: str) -> Tuple[str, str]:
        """
        Pick the single best weapon for a given situation.
        situation: "default" | "speed" | "heavy" | "precision"
          default   — best overall weapon
          speed     — lightest+fastest (conserve endurance when tired)
          heavy     — maximum damage output (foe is vulnerable, press the kill)
          precision — fast and accurate (careful finish on a damaged foe)
        """
        w = self.warrior
        mods = w.race.modifiers
        preferred_set = set(mods.preferred_weapons)

        eligible = self._eligible_weapons()
        if not eligible:
            return ("Short Sword", "default choice")

        scored = []
        for key, wpn, two_h_cap in eligible:
            name = wpn.display
            score = 0

            # Racial preference + skill (baseline for every situation)
            if name in preferred_set:
                score += 20
            score += w.skills.get(key, 0) * 3

            # STR over-cap penalty
            over = wpn.weight - two_h_cap
            if over > 0:
                score -= int(over * 8)

            # Racial specials (always relevant)
            if mods.thrown_mastery and wpn.throwable:
                score += 15
            if mods.natural_weapon_bonus and key == "open_hand":
                score += 20
            if mods.dual_weapon_bonus and wpn.weight <= 3.0 and not wpn.two_hand:
                score += 5

            # Situation-specific scoring
            if situation == "default":
                if wpn.weight < 2.5 and w.dexterity >= 14:
                    score += 10
                elif wpn.weight >= 4.0 and w.strength >= 14:
                    score += 10
                else:
                    score += 5

            elif situation == "speed":
                # When tired: lighter weight + higher APM saves endurance
                score += int((5.0 - min(wpn.weight, 5.0)) * 4)
                score += wpn.apm * 2

            elif situation == "heavy":
                # Press the kill: maximum damage potential
                score += wpn.damage_top
                if wpn.weight >= 3.0:
                    score += 5

            elif situation == "precision":
                # Careful finish: fast, accurate, controllable
                score += wpn.apm * 2
                if wpn.weight <= 4.0:
                    score += 8

            # Don't recommend bare fists (Open Hand) in situation picks unless
            # the race genuinely benefits from them
            if key == "open_hand" and not (mods.natural_weapon_bonus or mods.martial_combat_bonus):
                mc_skill = w.skills.get("martial_combat", 0)
                if mc_skill < 5:
                    score -= 50  # push it out of contention

            scored.append((score, name, wpn.weight, wpn.two_hand))

        scored.sort(key=lambda x: -x[0])
        _, name, weight, two_hand = scored[0]

        labels = {
            "default": "primary weapon",
            "speed":   f"wt {weight:.1f} — lightest good option, conserves endurance",
            "heavy":   f"maximum damage — press the kill",
            "precision": f"fast and accurate — controlled finish",
        }
        label = labels.get(situation, "situational")
        if two_hand:
            label += " (2H)"
        return (name, label)

    def recommend_weapons(self) -> List[Tuple[str, str]]:
        """Recommend top weapons using racial traits, skills, and attributes."""
        w = self.warrior
        mods = w.race.modifiers
        preferred_set = set(mods.preferred_weapons)

        scored = []
        for key, wpn, two_h_cap in self._eligible_weapons():
            name = wpn.display
            score = 0
            reasons = []
            ...
            if name in preferred_set:
                score += 30
                reasons.append("racially preferred")

            skill_level = w.skills.get(key, 0)
            if skill_level > 0:
                score += skill_level * 4
                reasons.append(f"skill {skill_level}/9")

            over = wpn.weight - two_h_cap
            if over > 0:
                score -= int(over * 8)
                reasons.append("STR slightly low")

            if wpn.weight < 2.5:
                if w.dexterity >= 14:
                    score += 12
                    reasons.append("suits DEX")
                elif w.dexterity >= 12:
                    score += 6
            elif wpn.weight >= 4.0:
                if w.strength >= 15:
                    score += 12
                    reasons.append("suits STR")
                elif w.strength >= 13:
                    score += 6
            else:
                if w.strength >= 13 or w.dexterity >= 13:
                    score += 8

            if mods.thrown_mastery and wpn.throwable:
                score += 20
                reasons.append("thrown mastery")
            if mods.natural_weapon_bonus and key == "open_hand":
                score += 25
                reasons.append("natural weapon bonus")
            if mods.dual_weapon_bonus and wpn.weight <= 3.0 and not wpn.two_hand:
                score += 8
                reasons.append("dual wield eligible")
            if mods.martial_combat_bonus and wpn.mc_compatible:
                score += 5
                reasons.append("MC compatible")
            if mods.counterstrike_mastery and wpn.weight <= 3.5:
                score += 5

            scored.append((score, name, reasons, wpn.weight, wpn.two_hand))

        scored.sort(key=lambda x: -x[0])

        recommendations = []
        for score, name, reasons, weight, two_hand in scored[:6]:
            label = ", ".join(reasons) if reasons else f"wt {weight:.1f} — balanced"
            if two_hand:
                label += " (2H)"
            recommendations.append((name, label))

        if not recommendations:
            recommendations.append(("Short Sword", "default — balanced"))
    ...
    def recommend_strategies(self) -> List[Dict]:
        ...
        strategies.append({
            ...
            "weapon": self._best_weapon_for_situation("default"),
        })
        ...
        strategies.append({
            ...
            "weapon": self._best_weapon_for_situation("speed"),
        })
        ...
        # Defensive styles → lighter weapon to stay mobile; aggressive → max damage
        dmg_wpn_sit = "speed" if damage_style in ("Parry", "Defend") else "heavy"
        strategies.append({
            ...
            "weapon": self._best_weapon_for_situation(dmg_wpn_sit),
        })
        ...
        strategies.append({
            ...
            "weapon": self._best_weapon_for_situation("heavy"),
        })
        ...
        strategies.append({
            ...
            "weapon": self._best_weapon_for_situation("precision"),
        })
...
""",
        ...
        # RACIAL TRAITS
        self.info_text.insert(tk.END, f"\nRACIAL TRAITS ({w.race.name}):\n")
        mods = w.race.modifiers
        trait_lines = []
        if mods.attack_rate_bonus:    trait_lines.append(f"  + Attack Rate +{mods.attack_rate_bonus}")
        if mods.attack_rate_penalty:  trait_lines.append(f"  - Attack Rate -{mods.attack_rate_penalty}")
        if mods.damage_bonus:         trait_lines.append(f"  + Damage +{mods.damage_bonus}")
        if mods.damage_penalty:       trait_lines.append(f"  - Damage -{mods.damage_penalty}")
        if mods.dodge_bonus:          trait_lines.append(f"  + Dodge +{mods.dodge_bonus}")
        if mods.dodge_penalty:        trait_lines.append(f"  - Dodge -{mods.dodge_penalty}")
        if mods.parry_bonus:          trait_lines.append(f"  + Parry +{mods.parry_bonus}")
        if mods.parry_penalty:        trait_lines.append(f"  - Parry -{mods.parry_penalty}")
        if mods.hp_bonus:             trait_lines.append(f"  {'+'if mods.hp_bonus>0 else ''} HP {mods.hp_bonus:+d}")
        if mods.initiative_bonus:     trait_lines.append(f"  + Initiative +{mods.initiative_bonus}")
        if mods.strength_penalty:     trait_lines.append(f"  - STR -{mods.strength_penalty}")
        if mods.heavy_weapon_penalty: trait_lines.append(f"  ⚠ Heavy weapon penalty (wt≥4.0 or 2H)")
        if mods.spear_exception:      trait_lines.append(f"  ✓ Spears exempt from heavy penalty")
        if mods.dual_weapon_bonus:    trait_lines.append(f"  + Dual weapon bonus")
        if mods.thrown_mastery:       trait_lines.append(f"  + Thrown mastery")
        if mods.counterstrike_mastery:trait_lines.append(f"  + Counterstrike mastery")
        if mods.natural_weapon_bonus: trait_lines.append(f"  + Natural weapon bonus (MC/claws)")
        if mods.martial_combat_bonus: trait_lines.append(f"  + Martial Combat bonus")
        if mods.frenzy_ability:       trait_lines.append(f"  + Frenzy ability")
        if mods.acrobatic_advantage:  trait_lines.append(f"  + Acrobatic advantage")
        if mods.trains_stats_faster:  trait_lines.append(f"  + Trains stats faster")
        if mods.preferred_weapons:    trait_lines.append(f"  ✓ Preferred: {', '.join(mods.preferred_weapons[:4])}")
        if mods.weak_weapons:         trait_lines.append(f"  ✗ Avoid: {', '.join(mods.weak_weapons[:4])}")
        for tl in trait_lines:
            self.info_text.insert(tk.END, tl + "\n")
        if not trait_lines:
            self.info_text.insert(tk.END, "  (No special modifiers)\n")
    ...
    def create_strategy_card(self, strat: Dict):
        ...
        # Weapon
        if strat.get("weapon"):
            wpn_name, wpn_reason = strat["weapon"]
            wpn_frame = ttk.Frame(frame)
            wpn_frame.pack(fill=tk.X, pady=3)
            ttk.Label(wpn_frame, text="Weapon:", font=("Arial", 10, "bold")).pack(
                side=tk.LEFT, padx=5
            )
            ttk.Label(
                wpn_frame,
                text=f"{wpn_name}  —  {wpn_reason}",
                font=("Arial", 10, "bold"),
