...
def _load_uploads(turn_num):
    ...
    for fname in sorted(os.listdir(td)):
        ...
        # Verify that this manager still officially owns this team ID
        if mid in mgrs:
            valid_tids = [str(t) for t in mgrs[mid].get("team_ids", [])]
            if team_id and str(team_id) not in valid_tids:
                continue  # Stale upload: manager no longer has this team
...
def _admin_page():
    ...
    for t in sorted(list_saved_teams(), key=lambda x: x["team_id"]):
        team_options += f'<option value="{t["team_id"]}">{t["team_name"]} (ID: {t["team_id"]}, Manager: {t["manager_name"]})</option>'
                ...
                if os.path.exists(td_path):
                    for fn in os.listdir(td_path):
                        ...
                        if is_match:
                            ...
                            cur_fn = f"upload_{mid}_team{team_id}.json" if team_id else f"upload_{mid}.json"
                            if fn == cur_fn: continue

                            if tids: # If using IDs, check ID match; no-ID file is stale
                                if not chk_tid or str(chk_tid) not in tids:
