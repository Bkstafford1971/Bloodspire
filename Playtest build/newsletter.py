...
def _team_standings(teams, turn_num: int) -> str:
    ...
    for i,(r,r5) in enumerate(zip(rows,rows_l5),1):
        ...
        cname  = f" {r['name']} ({B_RED}{r['mgr']}{RESET}) ({r['id']})"
        c5name = f" {r5['name']} ({B_RED}{r5['mgr']}{RESET}) ({r5['id']})"
...
def _warrior_tiers(teams, champion_state: dict, card: list = None, turn_num: int = 0) -> str:
    ...
    if card:
        for bout in card:
            ...
            if pw: fought_names.add(getattr(pw, "name", ""))
            if ow: fought_names.add(getattr(ow, "name", ""))
