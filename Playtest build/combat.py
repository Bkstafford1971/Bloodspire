...
def _calc_damage_hybrid(
    ...
    # --- Req 1: Small weapon precision logic ---
    if is_small:
        # Small weapons rely on DEX and Skill more than weight/STR
        base = 5.0 # Higher floor for small weapons
        base += (attacker.dexterity - 10) * 0.8
        base += wpn_skill * 1.5 # Skills are more valuable for precision weapons
        base += weapon.weight * 1.0
    else:
    ...
    floor_frac = 0.20 if (is_small and wpn_skill >= 5) else 0.10
...
def _update_endurance(
    state: _CState, strategy: Strategy, foe: _CState # Removed 'actions' parameter
) -> List[str]:
    lines  = []
    props  = get_style_props(strategy.style)
    ...
    state.endurance = max(0.0, min(100.0, state.endurance - burn)) # Apply burn for a single action
...
class CombatEngine:
    ...
    def _resolve_action(self, as_: _CState, ds_: _CState, ax: Strategy, dx: Strategy, minute: int, _dbg_init=None) -> Optional[FightResult]:
        ...
        for ln in _update_endurance(as_, ax, ds_): # ds_ is the foe for as_
            self._emit(ln)
    ...
    def _counterstrike(self, as_: _CState, ds_: _CState, ax: Strategy, dx: Strategy, minute: int) -> Optional[FightResult]:
        ...
        for ln in N.hit_line(att.name, dfr.name, wpn, cat, ax.aim_point, "precise", attacker_race=att.race.name):
            self._emit(ln)
        # Note: Counterstrike damage is a fixed margin, not a roll.
        ...
        if ds_.current_hp <= 0:
            self._check_and_switch_strategies(as_, ds_, minute) # Re-evaluate strategies before death/concede
