...
def _update_champion(teams, champion_state: dict, deaths_this_turn: list,
    ...
    if len(tied) > 1:
        ...
        if len(still) > 1:
            ...
            still.sort(key=lambda x: (x[0].name, x[2]))
...
def _team_standings(teams, turn_num: int) -> str:
    ...
    for team in teams:
        ...
        rows.append({"name":name,"id":tid,"mgr":mgr,"w":tw,"l":tl,"k":tk,"pct":pct,
                     "l5w":l5w,"l5l":l5l,"l5k":l5k,"l5pct":l5pct})
    ...
    for i,(r,r5) in enumerate(zip(rows,rows_l5),1):
        # Construct styled name strings
        # Format: Team Name (Manager) (ID)
        cname  = f" {_trunc(r['name'])} ({B_RED}{_trunc(r['mgr'])}{RESET}) ({r['id']})"
        c5name = f" {_trunc(r5['name'])} ({B_RED}{_trunc(r5['mgr'])}{RESET}) ({r5['id']})"
...
def _warrior_tiers(teams, champion_state: dict, card: list = None, turn_num: int = 0) -> str:
    ...
    for team in teams:
        ...
        for i, wobj in enumerate(wlist):
            ...
            # Mark warriors who did not fight this turn with a '-' prefix
            did_fight = (tid, wobj.name) in fought_names
            disp_name = wobj.name if did_fight else f"-{wobj.name}"

            is_champ = (wobj.name == champ_name and tid == champ_tid)
            tier = _warrior_tier(wobj, is_champ)
            tiers[tier].append({"name":disp_name,"team":tname,"tid":tid,
                "w":wobj.wins,"l":wobj.losses,"k":wobj.kills,
                "rec":getattr(wobj,"recognition",0),
            })
    SEP = "="*70
    # Fixed columns: name(20) + W(4) + L(4) + K(4) + Rec(4) + team
    COL_HDR = f"{'NAME':<20}{'W':>4}{'L':>4}{'K':>4}  {'REC':>3}  TEAM"
    sections=[]
    for tier in [TIER_CHAMPION,TIER_ELITES,TIER_VETERANS,TIER_ADEPTS,TIER_INITIATES,TIER_ROOKIES,TIER_RECRUITS]:
        ...
        for wd in wlist:
            ...
            lines.append(f"{_trunc(wd['name']):<20}{wd['w']:>4}{wd['l']:>4}{wd['k']:>4}"
                         f"  {wd['rec']:>3}  {tm}")
        sections.append("\n".join(lines))
...
def _dead_section(deaths: list, turn_num: int) -> str:
    ...
    sep="="*79
    lines=["\nTHE DEAD",
           f"{'NAME':<20}{'W':>4}{'L':>4}{'K':>4}  {'TEAM':<20}{'SLAIN BY':<20}{'TURN':>5}",sep]
    for d in deaths:
        name = _trunc(d['name'])
        team = _trunc(d.get('team','?'))
        slain = _trunc(d.get('killed_by','?'))
        lines.append(f"{name:<20}{d.get('w',0):>4}{d.get('l',0):>4}{d.get('k',0):>4}"
                     f"  {team:<20}{slain:<20}{turn_num:>5}")
    return "\n".join(lines)
...
def _race_report(teams) -> str:
    ...
    if top:
        lines.append("\n\n                      TOP WARRIOR by RACE\n")
        lines.append(f"{'RACE':<14}{'WARRIOR':<22}{'W':>4}{'L':>4}{'K':>3}  TEAM NAME"); lines.append(sep)
        for race in races:
            if race in top:
                td=top[race]
                lines.append(f"{race:<14}{_trunc(td['warrior']):<20}{td['w']:>4}{td['l']:>4}{td['k']:>3}"
...
def _top_managers(card, teams) -> str:
    """
    ...
    for team in teams:
        ...
        print(f"  [DEBUG] Processing manager: {mgr_name}")
    ...
    # Sort by win percentage (highest first), then by wins (tiebreaker)
    manager_list.sort(key=lambda x: (-x["pct"], -x["w"]))

    # Format output
    SEP = "="*70
    TITLE = "The Top Managers"
    HDR = f"{'MANAGER':<35}{'W':>4}{'L':>4}{'K':>4}{'%':>7}{'TOTAL':>6}"
