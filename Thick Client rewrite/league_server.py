...
from warrior import (
    generate_base_stats, assign_favorite_weapon, ATTRIBUTES, 
    TRIGGERS, FIGHTING_STYLES, AIM_DEFENSE_POINTS, 
...
def _run_turn(request_password, rerun_turn=None):
    ...
    with _lock:
        ...
        # Clear static rollups for all managers so they get fresh ones for the new turn
        mgrs = _load_managers()
        for m in mgrs.values(): m.pop("current_rollup", None)
        _save_managers(mgrs)
...
class LeagueHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/play":
            # Prefer the lowercase filename as it contains our architectural updates
            client_path = os.path.join(BASE_DIR, "bloodspire_client.html")
            if not os.path.exists(client_path):
                client_path = os.path.join(BASE_DIR, "Bloodspire_client.html")
            self.send_file(client_path, "text/html; charset=utf-8"); return
        ...
        if path.startswith("/static/"):
            ...
            self.send_file(fpath, ctype); return
        ...
        if path == "/api/results":
            ...
            # Ensure manager has a static rollup assigned
            if "current_rollup" not in mgrs[mid]:
                mgrs[mid]["current_rollup"] = [generate_base_stats() for _ in range(5)]
                _save_managers(mgrs)

            res_turn = cfg["current_turn"] - 1
            if res_turn < 1:
                self.send_json({
                    "success": True, "results": [], "turn": cfg["current_turn"], 
                    "has_newsletter": False, "rolls": mgrs[mid]["current_rollup"]
                }); return
            ...
            self.send_json({"success":True,"results":team_results, "turn":res_turn, 
                            "has_newsletter":bool(nl_text), "rolls": mgrs[mid]["current_rollup"]}); return
        ...
        if path == "/api/get_rollup":
            # Generate 5 sets of base stats for a new team
            rolls = [generate_base_stats() for _ in range(5)]
            self.send_json({"success": True, "rolls": rolls}); return
    ...
    def do_POST(self):
        ...
        if path in ("/api/account/create", "/api/register"):
            ...
            with _lock:
                mgrs = _load_managers()
                # Check for existing manager by name (case-insensitive)
                found_mid = None
                ...
                if found_mid:
                    # Verify password for existing account (effectively a login)
                    if _check_mgr_pw(mgrs[found_mid], pw):
                        m = mgrs[found_mid]
                        if "current_rollup" not in m:
                            m["current_rollup"] = [generate_base_stats() for _ in range(5)]
                            _save_managers(mgrs)

                        self.send_json({
                            "success": True,
                            "id": found_mid,
                            "manager_id": found_mid,
                            "manager_name": m["manager_name"],
                            "email": m.get("email", ""),
                            "team_ids": m.get("team_ids", []),
                            "run_next_turn": m.get("run_next_turn", {}),
                            "rolls": m["current_rollup"]
                        })
                    else:
                ...
                mgrs[mid] = {
                    ...
                    "team_ids": [],
                    "run_next_turn": {},
                    "current_rollup": [generate_base_stats() for _ in range(5)]
                }
                _save_managers(mgrs)
            self.send_json({
                "success": True,
        ...
        if path == "/api/account/login":
            ...
            pw = (b.get("password") or "").strip()
            mgrs = _load_managers()
            found_mid = next((mid for mid, m in mgrs.items() if m["manager_name"].lower() == mname.lower()), None)
            
            if found_mid and _check_mgr_pw(mgrs[found_mid], pw):
                m = mgrs[found_mid]
                if "current_rollup" not in m:
                    m["current_rollup"] = [generate_base_stats() for _ in range(5)]
                    _save_managers(mgrs)
                ...
                self.send_json({"success": False, "error": "Invalid manager name or password."}, 401); return
        ...
        if path == "/api/upload":
            ...
            # --- Finalize Warrior Attributes (Luck, Favorite Weapon, etc.) ---
            if isinstance(team, dict) and "warriors" in team:
                from warrior import Warrior as _W
                for wd in team["warriors"]:
                    if not wd: continue
                    # If this is a new warrior (missing server-assigned attributes)
                    if not wd.get("luck") or not wd.get("favorite_weapon"):
                        try:
                            temp_w = _W.from_dict(wd)
                            if not getattr(temp_w, "luck", 0):
                                import random; temp_w.luck = random.randint(1, 30)
                            if not getattr(temp_w, "favorite_weapon", ""):
                                assign_favorite_weapon(temp_w)
                            # Update the dictionary with server-assigned values
                            wd.update(temp_w.to_dict())
                            # Ensure height/weight are calculated based on server logic
                            h, w = temp_w._calc_measurements()
                            wd["height_in"] = h
                            wd["weight_lbs"] = w
                        except Exception as e:
                            print(f"Error finalizing warrior {wd.get('name')}: {e}")
...
def main():
    ...
    parser.add_argument("--host-password", required=False,
                        default="password",
                        help="Password for host admin access (defaults to 'password')")
