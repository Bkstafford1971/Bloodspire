...
def _save_upload(turn_num, manager_id, data):
    ...
    if r2_client.is_configured():
        r2_client.upload_team(turn_num, manager_id, team_id, data)
...
def _save_result(turn_num, manager_id, data):
    ...
    _save_json(os.path.join(_turn_dir(turn_num), fname), data)
    if r2_client.is_configured():
        r2_client.save_result(turn_num, manager_id, team_id, data)
...
def _run_turn(request_password, rerun_turn=None):
    ...
    with _lock:
        ...
        if r2_client.is_configured():
            uploads = r2_client.list_uploads(turn_num)
        else:
    ...
    with _lock:
        ...
        if not rerun_turn:
            ...
            if r2_client.is_configured():
                r2_client.delete_all_uploads(turn_num)
                r2_client.cleanup_old_results(turn_num)
                r2_client.archive_turn(turn_num, _turn_dir(turn_num))
    ...
    try:
        ...
        if r2_client.is_configured():
            r2_client.save_newsletter(turn_num, newsletter_text)
...
<div class="bar">⚔ BLOODSPIRE League — Admin
 ...
 <span id="r2-badge" style="{'display:inline-block' if r2_client.is_configured() else 'display:none'};background:#007acc;color:#fff;padding:2px 8px;border-radius:3px;font-size:10px;margin-left:8px;vertical-align:middle;border:1px solid #005f99">R2 ACTIVE</span>
...
<div class="wrap">
 <div class="panel" style="min-width:260px;max-width:340px">
  ...
  <button onclick="cleanupR2Results()" style="background:#eee;border-color:#ccc;color:#333;margin-left:4px" title="Manual R2 Cleanup">🧹</button>
 ...
 <div class="panel" style="min-width:240px;max-width:340px">
  ...
  <div id="sched-details" style="margin-top:8px;padding-left:4px">
   ...
   <select id="sched-day" onchange="saveSchedule()" style="font-size:12px;border:2px inset #808080;margin-left:4px">
    {''.join(f'<option {"selected" if cfg.get("schedule_day")==d else ""}>{d}</option>' for d in ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"])}
   </select>
   <br><br>
   Time (24h):
   <input type="time" id="sched-time" onchange="saveSchedule()"
          ...
          value="{cfg.get('schedule_time','20:00')}">
...
async function saveSchedule(){{
  ...
  const day=document.getElementById('sched-day')?.value||'Friday';
  const time=document.getElementById('sched-time')?.value||'20:00';
  try{{
   const r=await fetch('/api/admin/update',{{method:'POST',
    headers:{{'Content-Type':'application/json'}},
    body:JSON.stringify({{host_password:pw,schedule_enabled:enabled,schedule_day:day,schedule_time:time}})}});
   const d=await r.json();
   if(d.success){{show(`Saved schedule: ${{enabled?'enabled':'disabled'}} — ${{day}} ${{time}}`,'ok'); await refreshScheduleStatus(); return true;}}
...
async function refreshScheduleStatus(){{
  try{{
   ...
   const next=d.schedule_enabled ? `Next auto-run: ${{d.schedule_day}} ${{d.schedule_time}}` : 'Auto-schedule disabled';
   const last=d.schedule_last_run_turn ? `Last auto-run: turn ${{d.schedule_last_run_turn}}${{d.schedule_last_run_at?` at ${{d.schedule_last_run_at}}`:''}}${{d.schedule_last_run_result?` — ${{d.schedule_last_run_result}}`:''}}` : 'Last auto-run: never';
   el.textContent=`${{next}} | ${{last}}`;
...
class LeagueHandler(http.server.BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        """Standard logging enabled to help debug connectivity."""
        # Try to get the real IP from Cloudflare/Proxy headers
        ip = self.headers.get('Cf-Connecting-Ip') or self.headers.get('X-Forwarded-For') or self.address_string()
    ...
    def _cors(self):
        ...
        # Allow more headers to prevent Cloudflare/Proxy preflight blocks
    ...
    def do_GET(self):
        ...
        if ext in (".html", ".png", ".jpg", ".jpeg", ".ico", ".gif") or asset_name == "bloodspire.png":
            fpath = os.path.join(BASE_DIR, asset_name)
        ...
        if path == "/api/results":
                ...
                for fname in sorted(os.listdir(td)):
                    if fname.startswith(f"result_{mid}") and fname.endswith(".json"):
                        ...
                        if r:
                            if r2_client.is_configured():
                                tid = r.get("team_id", "")
                                r2_fname = f"result_{mid}_team{tid}.json" if tid else f"result_{mid}.json"
                                r["r2_url"] = r2_client.get_presigned_url(f"results/{res_turn}/{r2_fname}")
            ...
            nl_url = None
            if r2_client.is_configured() and nl_text:
                nl_url = r2_client.get_newsletter_url(res_turn)
            ...
            self.send_json({"success":True,"results":team_results,
                            "turn":res_turn,
                            "has_newsletter":bool(nl_text),
                            "newsletter_url": nl_url}); return
    ...
    def do_POST(self):
        ...
        if path == "/api/admin/cleanup_r2":
            cfg = _load_config()
            if not _check_host_pw(cfg, b.get("host_password","")):
                self.send_json({"success":False,"error":"Not authorised."}, 401); return
            if r2_client.is_configured():
                r2_client.cleanup_old_results(cfg["current_turn"])
                self.send_json({"success":True,"message":"Old results cleaned up from R2."})
            else:
                self.send_json({"success":False,"error":"R2 is not configured."})
            return
...
def main():
    ...
    print(f"\n  R2 STORAGE  :  {'ENABLED (bloodspire-arena)' if r2_client.is_configured() else 'DISABLED'}")
    print(f"\n  [Setup Guide]")
    print(f"  1. Share your Public IP or Cloudflare Tunnel URL with players.")
    print(f"  2. Players enter that address as the 'Server URL' in their client.\n")
        ...
        while True:
            ...
            try:
                ...
                if (cur_day == day_target
                    ...
                    cfg["schedule_last_run_at"] = now.strftime("%Y-%m-%d %H:%M:%S")
                    cfg["schedule_last_run_turn"] = cur_turn
                    cfg["schedule_last_run_result"] = f"Auto-run started on {cur_day} at {cur_time}"
                    _save_config(cfg)
